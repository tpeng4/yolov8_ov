...
void YOLOV8::detect(Mat & frame) {
    ...
    std::cout << "before preprocess_img" << std::endl;
    preprocess_img(frame);
    std::cout << "preprocess_img" << std::endl;
    infer_request.infer();

    std::cout << "infer" << std::endl;
    const ov::Tensor& output_tensor = infer_request.get_output_tensor();
    std::cout << "get_output_tensor" << std::endl;
    ov::Shape output_shape = output_tensor.get_shape();
    std::cout << output_shape << std::endl;
    float* detections = output_tensor.data<float>();
    this->postprocess_img(frame, detections, output_shape);
    std::cout << "postprocess_img" << std::endl;
}
...
void YOLOV8::initialmodel() {
    ...
    std::cout << "after reading model" << std::endl;
